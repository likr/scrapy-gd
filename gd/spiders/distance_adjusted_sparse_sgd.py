...
class DistanceAdjustedSparseSgdSpider(scrapy.Spider):
    name = "distance_adjusted_sparse_sgd"

    def __init__(self, graph=None, unit_edge_length=1, iterations=15, eps=0.1, l=0.5, pivot=200, seed_start=0, seed_stop=100, *args, **kwargs):
        super(DistanceAdjustedSparseSgdSpider, self).__init__(*args, **kwargs)
        self.graph = graph
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        spider = super(DistanceAdjustedSparseSgdSpider, cls).from_crawler(crawler, *args, **kwargs)
    ...
    def parse(self, response):
        ...
        for s in range(self.seed_start, self.seed_stop):
            drawing = layout_da_sparse(graph, distance, unit_edge_length=self.unit_edge_length, iterations=self.params['iterations'], eps=self.params['eps'], l=self.params['l'], pivot=self.params['pivot'], seed=s)
